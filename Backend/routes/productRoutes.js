...
router.post('/api/analyze', async (req, res) => {
    try {
        ...
        const commitsScore = Math.log10(commitsResponse.data.length + 1) / Math.log10(5000);
        ...
        const metrics = {
            ...
            popularityScore: 100 * (0.5 * starsScore + 0.3 * forksScore + 0.2 * commitsScore)
        }

        const {average} = complexity;
        const complexityScore = (1 - average) * 100

        const overallScore = 0.5 * codeQuality.codeQualityScore + 0.3 * metrics.popularityScore + 0.2 * complexityScore;
        ...
        if (err.response && err.response.status == 403 && err.response.data.message.includes('API rate limit exceeded')) {
